...
train_dataset_trg = torch.tensor (df1['Anti_label'][:num_train_data].tolist())
test_dataset_trg = torch.tensor (df1['Anti_label'][num_train_data:].tolist())
...
def draw_feature(feature, save_dir = 'c:\\Users\\G\\OneDrive\\바탕 화면\\KIST\\code\\reward_function_20250215\\3. result\\'):
...
for feature in tqdm(train_source_pc_input.columns[:-1]):
    sub_result_df = draw_feature(feature, save_dir ='c:\\Users\\G\\OneDrive\\바탕 화면\\KIST\\code\\reward_function_20250215\\3. result\\')
    result_df = pd.concat([result_df, sub_result_df])

result_df.to_excel('c:\\Users\\G\\OneDrive\\바탕 화면\\KIST\\code\\reward_function_20250215\\result_df.xlsx', index= False)
# %%

...
PGM_PATH= 'c:\\Users\\G\\OneDrive\\바탕 화면\\KIST\\code\\PeptoidGen_ver2025\\2. pgm'
RESULT_PATH = 'c:\\Users\\G\\OneDrive\\바탕 화면\\KIST\\code\\PeptoidGen_ver2025\\3. result'
...
sampled_item_pc_list = pd.concat(sampled_item_pc_list)
#%%
# sampled_item_pc_list= []
...
#reward_original_seq = reward_predict( pd.concat(sampled_item_pc_list) )
anti_result, hemodel_result = reward_predict( pd.concat(sampled_item_pc_list) )
...
train_dataloader_src_pc = DataLoader(torch.tensor(np.array(sampled_item_pc_list)), batch_size=batch_size, shuffle=False)
train_dataloader_src = DataLoader(train_dataset_src, batch_size=batch_size, shuffle=False)
train_dataloader_trg = DataLoader(train_dataset_trg, batch_size=batch_size, shuffle=False)
# %%
...
#%%
def train(target_network, behavior_network, train_dataloader_src,  train_dataloader_trg,  train_dataloader_src_pc, optimizer, criterion, clip, epoch, n_epochs, memory, seq_len, reward_threshold, bad_sample_reward):
    ...
    for (batch1, indices1), (batch2, indices2), (batch3) in zip(train_dataloader_src, train_dataloader_trg, train_dataloader_src_pc):
        ...
        mask = torch.zeros(seq_len * actions.shape[1], dtype = torch.float64)  # pad token 에 0 곱해
        ...
        while i < len(token) :
            ...
            if (tokenizer.pad_token in datapoint_token) and (tokenizer.sep_token in datapoint_token): 
                ...
                mask[i:i+stop_idx] = torch.ones(stop_idx, dtype = torch.float64)      
            ...
            # pad token 만 있을 경우
                ...
                mask[i:i + pad_location] = torch.ones(pad_location, dtype = torch.float64)
                ...
                actions[i:i+seq_len] = torch.tensor(datapoint)
                sample_for_buffer[i:i+seq_len] = torch.tensor( [tokenizer.cls_token_id]  + datapoint[:-1]  )
                mask[i:i + mask_location] = torch.ones(mask_location, dtype = torch.float64)
            # END token (NH2) 만 있을 경우
                ...
                actions[i:i+seq_len] = torch.tensor(datapoint)
                sample_for_buffer[i:i+seq_len] = torch.tensor( [tokenizer.cls_token_id] + datapoint[:-1])
                mask[i:i + end_location] = torch.ones(end_location, dtype = torch.float64)
                ...
                mask[i:i+ seq_len] = torch.ones(len(datapoint), dtype = torch.float64)
        ...
        Entropy =  - torch.sum(output * torch.log(output+ 1e-10), dim=1).mean()
...
class Buffer():
    ...
    def put(self, epoch, sequence_original, generated_sequence, reward_generated_sequence, idx):
        ...
        maximal = torch.max(reward_comparison, axis =1).values.type(torch.float64)
...
#%%
N_EPOCHS = 1500
reward_threshold_list = np.arange(0, 0.5 , 0.003).tolist() + [0.5] * 5000
behavior_network = model
target_network = model
interval = 1
memory = Buffer()
bad_sample_reward = torch.zeros(size = (3000,3))
...
for epoch in tqdm(range(N_EPOCHS)):
    ...
    reward_threshold = reward_threshold_list[epoch]
    
    train_loss, train_reward, train_reward_std, len_good_sample_idx, memory, bad_sample_reward = train(target_network, target_network, train_dataloader_src, train_dataloader_trg, train_dataloader_src_pc, optimizer, criterion, CLIP, epoch, N_EPOCHS, memory, seq_len, reward_threshold, bad_sample_reward )
    ...
    if epoch > 10 : 
        if train_reward == torch.max(torch.tensor(reward_list[:])).item() :
            ...
            torch.save(model.state_dict(), os.path.join( DATA_PATH, f'saved_model/TFR_best_REINFORCED/sum/sum_2506_epoch_{epoch}.pt'))
        #elif epoch in [79,80]:
        #    print("Model weight saved")
    ...
    if epoch == N_EPOCHS -1:
        ...
        torch.save(model.state_dict(), os.path.join( DATA_PATH, f'saved_model/TFR_best_REINFORCED/sum/sum_250604_epoch_{epoch}.pt') )       
